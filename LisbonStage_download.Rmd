...
sensors <- c("1") # Water Temperature Celsius and Turbidity
interval <- "E" # Event = every 15 minutes

### Download data, bind, write --------------------------------------------
Stage <- lapply(stations, 
                 function(x){
                   cdec_query(station = "LIS",
                              sensor_num = sensors,
                              dur_code = interval,
                              start_date = start,
                              end_date = end)
...
Stage_df <- bind_rows(Stage) # bind rows into data frame
write_csv(Stage_df, "data/LisbonStage.csv")
```
